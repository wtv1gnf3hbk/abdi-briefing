 ...
 * for the Levant Briefing (abdi-briefing) refresh button.
 *
 * Deploy with:
 ...
 *
 * The GITHUB_TOKEN needs `repo` scope (or fine-grained: Actions read/write).
 */

const GITHUB_REPO = 'wtv1gnf3hbk/abdi-briefing';
  ...
  return await fetch(
    `https://api.github.com/repos/${GITHUB_REPO}/actions/workflows/${WORKFLOW_FILE}/dispatches`,
    {
      ...
      headers: {
        ...
        'User-Agent': 'abdi-briefing-worker'
      },
...
// CORS headers — allow the GitHub Pages site to call this worker
const corsHeaders = {
...
export default {
  async fetch(request, env) {
    ...
    try {
      // POST /trigger — Trigger the workflow via workflow_dispatch
      ...
      // GET /runs — Get latest workflow run (for polling status)
      ...
      // GET /status/:runId — Get specific run status
